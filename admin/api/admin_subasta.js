...
app.post("/update_user", upload.single('image'),function(req,res){
 ...
 if(req.file){
  req.body.file='subasta_assets/images/'+req.file.filename;
    var sql = "UPDATE users set  name =? ,image=?,email=?, mobile_no=?  WHERE user_pub_id = ?";
    var query = db.query(sql, [req.body.name,req.body.file,req.body.email, req.body.mobile_no, req.body.user_pub_id], function(err, result) {
   if (err)
  {
    //req.flash("msg", "Error In User update");
  console.log(err);
  ...
  else
  {
    ...
    res.redirect("/userDetails");
     ...
     var sql = "UPDATE users set  name =? ,email=?, mobile_no=? WHERE user_pub_id = ?";
    var query = db.query(sql, [req.body.name,req.body.email, req.body.mobile_no,  req.body.user_pub_id], function(err, result) {
   if (err)
  {
    //req.flash("msg", "Error In User update");
    console.log(err);
...
app.listen(3003,function(err){
  if(!err){
    console.log('listening port no 3003');
  }
})